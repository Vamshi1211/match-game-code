...
class App extends Component {
  ...
  clickTabItem = tabIdValue => {
    this.setState({activeId: tabIdValue})
  }
  ...
  thumbnailClicked = id => {
    ...
    if (activeImageId === id) {
      ...
      this.setState(prevState => ({
        ...
        activeImageUrl: imagesList[randomNumber].imageUrl,
        activeImageId: imagesList[randomNumber].id,
        isIdFound: false,
      }))
    } else {
      this.setState({isIdFound: true, timer: 0, reset: false})
    }
  }

  filteredList = () => {
    const {activeId} = this.state
    const filteredLstResults = imagesList.filter(
      eachItem => eachItem.category === activeId,
    )
    return filteredLstResults
  }
  ...
  getGameView = filteredList => {
    const {activeImageUrl, activeId} = this.state

    return (
      <div className="app-container">
        <img className="image" src={activeImageUrl} alt="match" />
  ...
  render() {
    ...
    let gameViewAndResultView

    if (timer === 0 || isIdFound || reset === false) {
      clearInterval(this.timerId)
      gameViewAndResultView = (
        <GameResultView
          scoreValue={score}
      ...
      gameViewAndResultView = this.getGameView(filteredList)
    }

    return (
      <div className="bg-container">
        <NavBarHeader scoreValue={score} timerValue={timer} />
        <div className="fruits-app-container">{gameViewAndResultView}</div>
      </div>
    )
  }
}
